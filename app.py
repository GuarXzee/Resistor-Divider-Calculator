...
class CircuitWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.Vout is not None:
            ...
            painter.setFont(QFont("Arial", 9, QFont.Bold))
            painter.drawText(mid_x + 70, mid_y + 5, f"{self.Vout:.2f} V")
...
class ResistorDividerApp(QWidget):
    ...
    def prepare_pairs(self):
        """Generate resistor pairs with 3-decimal precision in background"""
    ...
    def calculate_divider(self):
        ...
        Rup_values = np.array([p["Req"] for p in self.pairs])
        Rdn_values = np.array([p["Req"] for p in self.pairs])

        Rup_grid, Rdn_grid = np.meshgrid(Rup_values, Rdn_values, indexing="ij")

        Vout_grid = Vdn + (Rdn_grid / (Rup_grid + Rdn_grid)) * (Vup - Vdn)
        error_grid = np.abs(Vout_grid - Vout_target)

        min_idx = np.unravel_index(np.argmin(error_grid), error_grid.shape)
        best_up = self.pairs[min_idx[0]]
        best_dn = self.pairs[min_idx[1]]
        best_vout = Vout_grid[min_idx]
        best_error = error_grid[min_idx]

        # --- Show results ---
        self.result_area.setText(
            f"Best Match:\n"
            f" Rup1 = {best_up['R1']:.2f} Ω, Rup2 = {best_up['R2'] if pd.notna(best_up['R2']) else '-'}\n"
            f" Rdn1 = {best_dn['R1']:.2f} Ω, Rdn2 = {best_dn['R2'] if pd.notna(best_dn['R2']) else '-'}\n"
            f" Req_up = {best_up['Req']:.2f} Ω\n"
            f" Req_dn = {best_dn['Req']:.2f} Ω\n"
            f" Vout ≈ {best_vout:.2f} V (target {Vout_target:.2f} V)\n"
            f" Error = {best_error:.2f} V"
        )

        # Update schematic drawing
        self.circuit.set_values(best_up, best_dn, Vup, Vdn, best_vout)
